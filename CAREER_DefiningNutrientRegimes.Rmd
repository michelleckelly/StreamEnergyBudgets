...
Nprocess <- 
  left_join(Nprocess, modelResults_all, by = c("date", 
                                               "StreamPULSE_SiteID" = "site"))
...
```{r plotNProcessRates, fig.cap="Nitrogen fixation and denitrification rates measured on the dominant substrate at each study site.", echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width = 8, fig.pos="center"}
Nprocess$Process <- 
  factor(Nprocess$Process, levels = c("NFixation", "UnamendedDenit"),
         labels = c("Nitrogen Fixation", "Unamended Denitrification"))

Nprocess_sitemean <- 
  Nprocess %>%
  group_by(StreamPULSE_SiteID, date, Process) %>%
  summarise(ProcessRate_mean = mean(ProcessRate)) 

ggplot() +
  geom_point(data = Nprocess, aes(x = StreamPULSE_SiteID, y = ProcessRate, 
                                  fill = Substrate), size = 3, shape = 21) +
  geom_boxplot(data = Nprocess, aes(x = StreamPULSE_SiteID, y = ProcessRate), 
               fill = NA) +
  # Facet wrap plots
...
ggplot(data = Nprocess[!is.na(Nprocess$StreamPULSE_SiteID),], 
  aes(x = StreamPULSE_SiteID, y = ProcessRate, fill = Substrate)) +
  # Facet wrap plots
  facet_wrap(~Process, ncol = 2, scales = "free_y") +
  # Barplot
  ...
  # Error bars
  geom_errorbar(aes(ymin = ProcessRate, ymax = ProcessRate+SE), 
...
# Subset numeric data
Nprocess_num <- Nprocess_wide[-c(1:4, 6:7, 9:11, 12, 13, 15:16)]
# Plot correlation
corrplot::corrplot(cor(Nprocess_num, use = "complete.obs"), order = "hclust", 
                   type = "upper", tl.col = "black", addgrid.col = F)
```
...
ggplot(data = Nprocess) +
  geom_point(aes(y = NEP, x = ProcessRate, 
