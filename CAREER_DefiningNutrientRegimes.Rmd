...
modelResults_monthly <-
  ...
  summarise(GPP_sd = sd(GPP, na.rm = T), 
            ...
            NEP_mean = mean(GPP - ER, na.rm = T),
            NEP_sd = sd(GPP - ER, na.rm = T))
...
```{r plotNEP, fig.cap="Average daily NEP at each site for period of record (A) and Mean monthly NEP at each site for the period of record. Ribbon extends to mean plus one standard deviation and mean minus one standard deviation (B). DOY = day of year. (Would be neat to have a grey line for each year of data, instead of one line for the mean as is currently, and display trend with a local regression line)", echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width = 8, fig.pos="center"}
