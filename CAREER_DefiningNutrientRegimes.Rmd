...
Nprocess <- left_join(Nprocess, sites[c(1,4)], by = "SiteName")
...
```{r plotNEP, fig.cap="Average daily NEP at each site for period of record (A) and Mean monthly NEP at each site for the period of record. Ribbon extends to mean plus one standard deviation and mean minus one standard deviation (B). (Would be neat to have a grey line for each year of data, instead of one line for the mean as is currently, and display trend with a local regression line)", echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width = 9, fig.pos="center"}
...
ggplot(data = Nprocess) +
  ...
  #               color = site, fill = site, shape = Substrate), size = 4) +
  # Facet wrap plots
  facet_wrap(~Process+site, nrow = 3) +
  # Theme adjustments
  theme_classic() +
  theme(axis.text = element_text(color = "black"), 
        panel.background = element_rect(color = "black"), 
...
```{r plotGPPvsERFaceted, fig.cap="Fingerprint of the distribution of daily mean GPP against daily mean ER at each site. Dashed line represents 1:1 GPP:ER relationship.", echo = FALSE, message = FALSE, warning = FALSE, fig.height=3.5, fig.width = 8, fig.pos="center"}
...
```{r plotGPPvsER, fig.cap="Fingerprint of the distribution of daily mean GPP against daily mean ER at each site. Dashed line represents 1:1 GPP:ER relationship.", echo = FALSE, message = FALSE, warning = FALSE, fig.pos="center", fig.height=5, fig.width = 8}
