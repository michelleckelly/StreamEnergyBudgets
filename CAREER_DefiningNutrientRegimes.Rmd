...
############## Load LINXII data ############## 
sites_LINXII <- read_csv(file = "data/LINXII_Denit_Mulholland2009.csv",
                         na = "NaN") %>% 
...
Arikaree River, CO_ARIK-down (first pass done)
...
Red Butte Creek, UT_REDB-up (first pass done)

Pringle Creek, TX_PRIN-up (first pass done)

McDiffet Creek, KS_MCDI-up (first pass done)

Kings Creek, KS_KING-up (retry seperately, something weird happened)
...
sites <- full_join(sites, sites_LINXII) %>% filter(Source != "Other")
