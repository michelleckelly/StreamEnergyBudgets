...
#sum(CAREER$DenitAmended_Mean_uMolN2Om2hr <= 0, na.rm = T) / length(CAREER$DenitAmended_Mean_uMolN2Om2hr)
CAREER$DenitAmended_SD_uMolN2Om2hr[CAREER$DenitAmended_Mean_uMolN2Om2hr <= 0] <- NA
CAREER$DenitAmended_Mean_uMolN2Om2hr[CAREER$DenitAmended_Mean_uMolN2Om2hr <= 0] <- NA
...
# Amended
CAREER$DenitAmended_Mean_mgNm2hr <- 
  CAREER$DenitAmended_Mean_uMolN2Om2hr * 2 / 10^3 * 14
CAREER$EnergyDenitAmended_kCalm2hr <- 
  convertDenit(CAREER$DenitAmended_Mean_mgNm2hr)
# Totals
CAREER$EnergyTot_kCalm2hr <- 
  CAREER$EnergyDenitAmended_kCalm2hr + CAREER$EnergyER_kCalm2hr
CAREER$EnergyDenit_percent <- 
  CAREER$EnergyDenitAmended_kCalm2hr / CAREER$EnergyTot_kCalm2hr * 100
...
Denitrification was measured on individual stream substrates using **amended acetylene inhibition** assays.  
...
LINXIIRMSE %>%
  ...
  kable(caption = "Summary of model performance on testing set, where the MARS model has the best performance based on the lowest root mean squared error. RMSE = root mean squared error, MAE = mean absolute error.") %>%
  kable_styling(position = "left", bootstrap_options = c("striped", "hover"), 
                full_width = TRUE) %>%
  column_spec(5, bold = T, color = "white", background = "#D90734")
```
...
ggplot(data = subset(CAREER, !is.na(DenitAmended_Mean_mgNm2hr)), 
  ...
  # Label with site names
  #geom_text_repel(label = subset(CAREER, 
  #                               !is.na(DenitAmended_Mean_mgNm2hr))$SiteName, 
  #                size = 3) + 
  #geom_text_repel(label = subset(CAREER, 
  #                               !is.na(DenitAmended_Mean_mgNm2hr))$NO2NO3_ugL, 
  #                size = 3) + 
  # Theme adjustments
  theme_classic() +
  labs(x = expression("Denitrification rate (mg-N" ~ m^{-2} ~ hr^{-1} * ")"), 
...
ggplot(data = subset(CAREER, !is.na(DenitAmended_Mean_mgNm2hr)),
       aes(y = DenitAmended_Mean_mgNm2hr, x = Substrate)) +
...
1. Remaining **metabolism modeling** for Marcarelli Nationwide Survey (n = 24 sites, including 14 NEON Network sites)  

2. **Scaling** Marcarelli Nationwide chamber rates to **whole-stream** estimates at NEON sites, using **NEON stream morphology data products**  

3. **Exploring** factors that impact the **energy production from denitrification** in each **substrate**  
